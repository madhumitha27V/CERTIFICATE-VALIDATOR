...
# CRITICAL: Set Tesseract environment variable FIRST before any imports
# This must be set before importing admin or user apps
persistent_data_dir = '/opt/render/project/src/data'
if os.path.exists(persistent_data_dir):
    # Running on Render - find working Tesseract path
    tesseract_paths = ['/usr/bin/tesseract', '/usr/local/bin/tesseract', 'tesseract']
    working_tesseract = None
    ...
    if working_tesseract:
        os.environ['TESSERACT_CMD'] = working_tesseract
        print(f"🔧 Set TESSERACT_CMD to {working_tesseract} for Render deployment")
    else:
        os.environ['TESSERACT_CMD'] = '/usr/bin/tesseract'  # fallback
        print(f"⚠️ No working Tesseract found, using fallback: /usr/bin/tesseract")
else:
    # Running locally - keep Windows path if not already set
    if 'TESSERACT_CMD' not in os.environ:
        os.environ['TESSERACT_CMD'] = r'F:\Tesseract-OCR\tesseract.exe'
    print(f"🔧 Local development - TESSERACT_CMD: {os.environ.get('TESSERACT_CMD')}")
